					...
					<div class="outter-wp">
						...
						<div class="sub-heard-part">
							<ol class="breadcrumb m-b-0">
								...
								<li>
									<a href="<%=basePath%>meeting/queryMeetingByMyScheduled">我的预约</a>
								</li>
								<li>
									<a href="<%=basePath%>meeting/queryMeetingDetailAndResourceById?meetingId=${meeting.mId}&action=4">会议详情</a>
								</li>
								<li class="active">
									修改会议
								</li>
						...
						<div class="inbox-mail">
							...
							<div class="col-md-4 compose">
								...
								<fmt:parseDate var="startObj" value="${meeting.startTime}"
									type="DATE" pattern="yyyy-MM-dd HH:mm" />
								<fmt:formatDate var="startTime" value='${startObj}'
									pattern='yyyy-MM-dd HH:mm' />
								<fmt:parseDate var="endObj" value="${meeting.endTime}"
									type="DATE" pattern="yyyy-MM-dd HH:mm" />
								...
								<fmt:formatDate var="scheduledTime" value='${scheduledObj}'
									pattern='yyyy-MM-dd HH:mm' />
								<input type="hidden" id="oldST" value="${startTime}">
								<input type="hidden" id="oldET" value="${endTime}">
								<!-- search -->
								<div>
									<form onsubmit="return false" method="Post" id="roomform">
									    <input type="hidden" id="mId" name="mId" value="${meeting.mId}">
...
function checkAndQueryMR() {
	...
	$.ajax({
		url : "<%=basePath%>meetingRoom/queryMeetingRoomsToScheduledWithouMid",
...
$("#endTime").click(function() {
	...
	laydate(obj);
								...
								<ul class="nav tabs">
									...
									<li class="">
										<a> <strong>会议室:&nbsp;&nbsp;&nbsp;&nbsp;</strong>
										    <button class='label label-warning' title='撤销修改' onclick="restoreMR()">
										    撤销
										    </button>
								...
								<input type="hidden" id="mrId" name="mrId" value="${meeting.meetingRoom.mrId}">
								<input type="hidden" id="oldno" value="${meeting.meetingRoom.no}">
								<input type="hidden" id="oldmrid" value="${meeting.meetingRoom.mrId}">
								...
								<ul class="nav tabs">
									<div class="tables">
										<c:choose>
											<c:when test="${meetingResources.isEmpty()}">
												<li class="">
													<a><i>未借用资源</i> </a>
												</li>
											</c:when>
											<c:otherwise>
												<table class="table table-bordered" id="rtable">
													...
													<c:forEach var="meetingResource"
														items="${meetingResources}" varStatus="status">
														<tr id="${meetingResource.resource.rId}">
															<th scope="row" style="text-align: center">
																${status.count}
															</th>
															<td style="text-align: center">
																${meetingResource.resource.rName}
																<input type="hidden" value="${meetingResource.number}">
															</td>
															<td style="text-align: center"
																id="remain${meetingResource.resource.rId}">
																${meetingResource.resource.remain +
																meetingResource.number}
															</td>
															<td style="text-align: center">
																<input type="text" class="form-control"
																	id="number${meetingResource.resource.rId}"
																	placeholder="数量" title="输入借用数量"
																	onblur="checkNumber('${meetingResource.resource.rId}')"
																	value="${meetingResource.number}">
															</td>
														</tr>
													</c:forEach>
												</table>
										...
										<c:choose>
											<c:when test="${unAddResources.isEmpty()}">
												<li class="">
													<a><i>无可借资源</i> </a>
												</li>
											</c:when>
											<c:otherwise>
												<table class="table table-bordered" id="newrtable">
													<tr>
														<th style="text-align: center; width: 10%">
															<strong>#</strong>
														</th>
													...
													<c:forEach var="unAddResource" items="${unAddResources}"
														varStatus="status">
														<tr id="${unAddResource.rId}">
															<th scope="row" style="text-align: center">
																${status.count}
															</th>
															<td style="text-align: center">
															...
															<td style="text-align: center"
																id="remain${unAddResource.rId}">
																${unAddResource.remain}
															</td>
															<td style="text-align: center">
																<input type="text" class="form-control"
																	id="number${unAddResource.rId}" placeholder="数量"
																	title="输入借用数量"
																	onblur="checkNumber('${unAddResource.rId}')">
															</td>
														</tr>
													</c:forEach>
												</table>
...
function restoreMR() {
	$("#mrId").val($("#oldmrid").val());
	$("#no").val($("#oldno").val());
	$("#startTime").val($("#oldST").val());
	$("#endTime").val($("#oldET").val());
}
</script>
...
$("#user-modal-button").hide();
											...
											<table class="table" id="mdtable">
												...
												<c:forEach var="meetingDetail" items="${meetingDetails}" varStatus="status">
												    <tr class="table-row" id="${meetingDetail.user.userId}" data-row="${status.count}">
														<td class="table-img" style="width: 17%">
														...
														<td style="width: 17%" id="r${meetingDetail.user.userId}">
															<c:choose>
															...
															<input type="hidden" id="oldrole${meetingDetail.user.userId}" value="${meetingDetail.role}">
															<select name="role" id="role" title="选择会议角色" 
															        style="height: 30px; margin-top: 8px" 
															        onchange="changeRole('${meetingDetail.user.userId}')">
																<option value="participant" id="participant${meetingDetail.user.userId}">
																	参与者
																</option> 
																<option value="recorder" id="recorder${meetingDetail.user.userId}">
																	记录员
																</option>
																<option value="master" id="master${meetingDetail.user.userId}">
																	主持人
																</option>
															</select>
														</td>
														<td style="width: 17%" id="o${meetingDetail.user.userId}">
															<c:choose>
															...
															<input type="hidden" id="oldopt${meetingDetail.user.userId}" value="${meetingDetail.optional}">
															<select name="optional" id="optional" title="选择是否必须参加" 
															        style="height: 30px; margin-top: 15px" 
															        onchange="changeOptinal('${meetingDetail.user.userId}')">
																<option value="yes" id="yes${meetingDetail.user.userId}">
																	可选的
																</option>
																<option value="no" id="no${meetingDetail.user.userId}">
																	必须
																</option>
															</select>
														</td>
...
<script type="text/javascript">
var array = new Array();  
<c:forEach items="${meetingDetails}" var="meetingDetail">  
array.push(${meetingDetail.user.userId}); //js中可以使用此标签，将EL表达式中的值push到数组中  
</c:forEach>
for(var i = 0; i < array.length; i++) {
	var uid = array[i];
    var rolevalue = $("#oldrole" + uid).val();
    $("#" + rolevalue + uid).attr("selected", "selected");
    
    var optvalue = $("#oldopt" + uid).val();
    $("#" + optvalue + uid).attr("selected", "selected");
}
											...
											<ul>
												...
												<li>
													...
													<textarea class="form-control" rows="5" cols="100"
														...
														onblur="checkMInfo()">${meeting.description}</textarea>
												</li>
											</ul>
										</div>
									</div>
...
function checkUsers() {
	...
	$.ajax({
		url : "<%=basePath%>user/queryAvailableUsersByIdsAndMId",
		data : 
...
function getMRs() {
	...
	$("#newrtable tr:gt(0)").each(function(i) {
		var rid = $(this).attr("id");
		var num = $(this).children("td:eq(2)").find("input").val();
		if (num != "" && eval(num) > 0) {
			var ujson = {
...
function checkAndSub() {
	...
	$.ajax({
		url : "<%=basePath%>meeting/modifyMeeting",
		data : 
