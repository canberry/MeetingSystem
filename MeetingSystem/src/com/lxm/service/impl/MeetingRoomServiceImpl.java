...
public class MeetingRoomServiceImpl implements MeetingRoomService {
	...
	public void removeMeetingRoom(int mrId, User user) {
		...
		String nowTime = DateUtil.formatDate(new java.util.Date(), Const.DATEFORMAT_HM);
		...
		for (Meeting meeting : meetings) {
			if (meeting.getCancel().equals(Const.CANCEL_NO)) {				
				...
				List<MeetingRoom> meetingRooms = queryMeetingRoomsToScheduled(startTime, endTime, capacity);
				System.out.println("new mrs: " + meetingRooms);
				Meeting md = meetingService.queryMeetingDetailAndResourceById(meeting.getmId());
				...
				if (meetingRooms.isEmpty()) {
					...
					"可容纳 " + newMeetingRoom.getCapacity() + " 人";
					Message message = new Message();
					message.setMessageName(messageName);
					message.setContent(content);
					message.setSendUser(user);
					...
					"    开始时间: " + meeting.getStartTime() + "," + 
					"    结束时间: " + meeting.getEndTime() + "." + 
					"    原会议室已经更换, 系统自动帮您选定新的会议室: " + newMeetingRoom.getNo() + ", " +
					"可容纳 " + newMeetingRoom.getCapacity() + " 人";
					message.setContent(newcontent);
					message.setReceiveUser(meeting.getScheduler());
					messageService.addMessage(message);
